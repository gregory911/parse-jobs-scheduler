...
const init = (opts) => {
  try {
    ...
    Parse.initialize(opts.appID, opts.jsKey, opts.masterKey);
    Parse.serverURL = opts.PARSE_SERVER_URL;
...
const performJob = async (jobName, params) => {
  try {
    const jobQuery = new Parse.Query('_JobStatus');
          jobQuery.equalTo("jobName", jobName);
          jobQuery.descending("createdAt");
    const latestJob = await jobQuery.first({useMasterKey: true});
...
export { init, recreateSchedule, destroySchedule }
